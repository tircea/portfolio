...
conn = sqlite3.connect('/text1.pdf.frequency_dict.db')
cursor = conn.cursor()


conn2 = sqlite3.connect('/text2.txt.frequency_dict.db')
...
aaa = ["numr", "prep", "adjf", "advb"]


for item in aaa:
   cursor.execute(f"SELECT Ni, XiNi, squaredSum FROM abs_{item} WHERE Xi IS NULL")
   [(data)] = cursor.fetchall()
   (sumNiX, sumXiNiX, x) = data
   midFreqX = sumXiNiX/sumNiX
   cursor2.execute(f"SELECT  Ni, XiNi, squaredSum FROM abs_{item} WHERE Xi IS NULL")
   [(data)] = cursor2.fetchall()
   (sumNiY, sumXiNiY, y) = data
   midFreqY = sumXiNiY/sumNiY
  
   moduleNum = midFreqX-midFreqY
   if moduleNum < 0:
       moduleNum = moduleNum * -1

   t =  (moduleNum) / math.sqrt(
       ((x  + y) / (sumNiX + sumNiY-2)) * ((sumNiX + sumNiY)/(sumNiX * sumNiY))
   )

   l = (t-2.04)/t
   print(f"t {item} = {t}")
   print(f"l {item} = {l}")
   cursor.execute("INSERT into value_l_t (pos, l, t) values (?, ?, ?)", [item, l, t])
   cursor2.execute("INSERT into value_l_t (pos, l, t) values (?, ?, ?)", [item, l, t])
   conn.commit()
   conn2.commit()
